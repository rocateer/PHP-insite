...
class Event_v_1_0_0 extends MY_Controller {

	function __construct(){
		parent::__construct();

		$this->load->model('event_v_1_0_0/model_event');
	}
  ...
  // 이벤트 리스트
	public function event_list() {
		...
		$result_list = $this->model_event->event_list($data);// 이벤트 리스트
	  $result_list_count = $this->model_event->event_list_count($data);// 이벤트 리스트 count
	  $total_page = ceil($result_list_count/$page_size);
		...
		foreach($result_list as $row) {
			$data_array[$x]['event_idx'] = $row->event_idx;
			$data_array[$x]['title'] = $row->title;
			$data_array[$x]['contents'] = $row->contents;
			$data_array[$x]['start_date'] = $row->start_date;
			$data_array[$x]['end_date'] = $row->end_date;
  ...
  // 이벤트 상세보기
	public function event_detail() {
		...
		$event_idx=($this->input->post('event_idx', TRUE) != '') ? $this->_escstr($this->input->post('event_idx', TRUE)) : '';

		$data['event_idx']=$event_idx;

		$result = $this->model_event->event_detail($data);// 이벤트 상세보기
		...
		if(count($result) == 0) {
			...
			$response->code = "1000";
			$response->code_msg = $this->global_msg->code_msg('1000');
			$response->event_idx = $result->event_idx;
