...
//본인인증폼
 public function member_auth(){
  $auth_type = $this->_input_check('auth_type',array());

  switch($auth_type){
    case '0': $urlCode = '003002'; break; // return url 회원가입페이지
    case '1': $urlCode = '003001'; break;
    default: $urlCode = '003002'; break;
  }
   ...
   //결과수신url
   $data['tr_url'] = THIS_DOMAIN.'/kmc_web_view/member_auth_apply?auth_type='.$auth_type;

   //모듈호출데이터
   $data['certMet'] = 'M'; // 본인확인 방법 'M': 휴대폰 본인확인, 'P': 공인인증서
   $data['cpId'] = 'DBUT1001'; // 회원사 ID
   $data['urlCode'] = $urlCode; // 서비스 호출 웹 페이지마다 등록된 코드 정보, 등록 된 URL CODE만 서비스 호출이 가능
