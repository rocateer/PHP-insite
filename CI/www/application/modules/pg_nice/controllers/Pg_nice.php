...
class Pg_nice extends MY_Controller {
  ...
  function __construct(){
    ...
    define("LIB_HOME",'/home/platform/www/pg_module/nice');
  ...
  public function pg_start() {
    ...
    $nicepay->m_Moid        = $order_number;                    // 상품주문번호
    $nicepay->m_Price       = $product_price;                   // 결제상품금액(1004)
    $nicepay->m_BuyerEmail  = $order_email;                    // 구매자메일주소
    $nicepay->m_BuyerName   = $order_name;                      // 구매자명
    $nicepay->m_BuyerTel    = $order_tel;                       // 구매자연락처
    $nicepay->m_GoodsName   = $product_name;                         // 결제상품명
    $goodsCnt               = "1";                                  // 결제상품개수
