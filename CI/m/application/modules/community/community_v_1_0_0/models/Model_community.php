	...
	public function community_list($data){
		...
		$sql = "SELECT
						...
						FROM
							...
							JOIN tbl_category_management b on b.category_management_idx =a.category_idx and b.del_yn='N' and b.state='1'
		...
		$i++;}
			...
			$sql = "SELECT
							...
							FROM
								tbl_board a
								join tbl_member d on d.member_idx=a.member_idx and d.del_yn='N'
								left JOIN tbl_category_management b on b.category_management_idx =a.category_idx and b.del_yn='N' and b.state='1'
								left join tbl_board_report as c on c.board_idx=a.board_idx and c.member_idx=$member_idx and c.del_yn='N'
								LEFT JOIN tbl_board_like t ON t.member_idx = '$member_idx' AND t.board_idx = a.board_idx and t.del_yn = 'N'
							WHERE
								a.board_type = $board_type
	...
	public function community_detail($data){
		...
		if($result['community_detail']->program_record!=''){
			...
			$sql = "SELECT
								...
								a.member_idx, 
								a.program_idx, 
								b.del_yn as program_del_yn, 
								b.display_yn as program_display_yn, 
								a.title, 
								a.img_path, 
								a.yoil, 
								DATE_FORMAT(a.s_date, '%Y.%m.%d') AS s_date,
	...
	public function report_reg_in($data){
		...
		if($type=='0'){
			...
			$this->query($sql,
				...
				$sql = "UPDATE
									tbl_board
								SET
									report_cnt = (select count(*) from tbl_board_report where board_idx=$board_idx and del_yn='N'),
		...
		";
				...
				$sql = "UPDATE
									tbl_board
								SET
									display_yn = if(report_cycle>=3,'N',display_yn),
									report_cycle = if(report_cycle>=3,0,report_cycle),
									upd_date = NOW()
								WHERE
									board_idx = ?
		";
				...
				$sql = "UPDATE
									...
									tbl_board b
								SET
									a.member_reported_cnt = a.member_reported_cnt+1,
									a.upd_date = NOW()
								WHERE
		...
		";
				...
				$sql = "INSERT INTO
					...
					$sql = "UPDATE
										tbl_board_reply				
									SET
										report_cnt = (select count(*) from tbl_board_reply_report where board_reply_idx=$board_reply_idx and del_yn='N'),
										report_cycle = report_cycle+1,
										upd_date = NOW()
									WHERE
										board_reply_idx = ?
		...
		$member_idx = $data['member_idx'];
			...
			$sql = "SELECT
							...
							WHERE
								...
								and del_yn='N'
								and member_idx!=$member_idx
								and member_idx not in (SELECT member_idx FROM tbl_board WHERE board_idx = $board_idx)
								GROUP BY member_idx
		";
	...
	*/
	...
	public function board_comment_list($data){
		...
		$sql = "SELECT
							...
							c.member_idx as board_member_idx,
							b.member_idx,
							b.member_state,
							b.member_nickname,
		...
		foreach ($result_list as $row) {
			...
			$data_array[$x]['board_member_idx']	= $row->board_member_idx;
			$data_array[$x]['member_idx']	= $row->member_idx;
			$data_array[$x]['idx']	= $row->idx;
			$data_array[$x]['member_state']	= $row->member_state;
